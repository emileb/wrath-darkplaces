...
*/
	...
	fd = FS_SysOpenFD(va(vabuf, sizeof(vabuf), "%s%s/%s", userdir, gamedirname1, CONFIGFILENAME), "a", false);
	if(fd >= 0)
		close(fd);
#else
...
// If the base directory is explicitly defined by the compilation process
	...
	else if (COM_CheckParm("-nohome") || gamemode == GAME_WRATH)
	...
	for (i = 1;i < com_argc && fs_numgamedirs < MAX_GAMEDIRS;i++)
	{
		...
		if (!strcmp (com_argv[i], "-game") && i < com_argc-1)
		{
			...
			if(!p)
				Con_Printf("WARNING: Nasty -game name?: %s\n", com_argv[i]);
