...
cvar_t r_glsl_postprocess_color_lut = {CVAR_SAVE, "r_glsl_postprocess_color_lut", "", "color lookup table for post, empty string for default"};
...
static void R_Bloom_StartFrame(void)
{
	 ...
	 || useviewfbo != (r_fb.fbo != 0))
	{
		...
		if (r_fb.screentexturewidth && r_fb.screentextureheight)
		{
			...
			r_fb.colortexture = R_LoadTexture2D(r_main_texturepool, "framebuffercolor", r_fb.screentexturewidth, r_fb.screentextureheight, NULL, r_fb.textype, TEXF_RENDERTARGET | TEXF_CLAMP, -1, NULL);
			if (useviewfbo)
			{
